...
def pyls_completions(config, document, position):
    ...
    if include_class_objects:
        for c in completions:
            if c.type == 'class':
                with JEDI_COMPLETION_LOCK:
                    completion_dict = _format_completion(c, False, asynchronous_labels=resolve_labels_asynchronously)
...
def _format_completion(d, include_params=True, asynchronous_labels=True):
    completion = {
